...
export default function PdfToJpgPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await uploadFile('/pdf-to-jpg', fd, setProgress);
      setResult(res);
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="text-center mb-8 animate-fade-in">
        <div className="w-14 h-14 mx-auto mb-4 flex items-center justify-center rounded-2xl bg-gradient-to-br from-amber-500 to-yellow-500 text-white shadow-lg shadow-amber-500/20">
          <FileImage className="w-7 h-7" />
        </div>
        <h1 className="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white mb-2">PDF to JPG</h1>
        <p className="text-slate-500 dark:text-slate-400">Convert PDF pages to high-quality JPG images</p>
      </div>
      ...
      {/* Progress */}
      <div className="mt-6">
        <ProgressBar progress={progress} status="Converting PDF to images..." show={loading} />
      </div>
      ...
      {result && (
        <div className="mt-8 animate-fade-in-up">
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/20 border border-green-200 dark:border-green-800 rounded-2xl p-6 text-center mb-6">
            ...
            <p className="text-green-700 dark:text-green-300 font-semibold text-lg">PDF converted to images!</p>
            <p className="text-green-600 dark:text-green-400 text-sm mt-1">{result.totalPages} page{result.totalPages !== 1 ? 's' : ''} converted</p>
          </div>
